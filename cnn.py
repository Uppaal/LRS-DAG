...
class Net(nn.Module):
    def __init__(self):
        ...
        self.N1 = nn.Sequential(nn.Conv2d(1, 1, 3, stride=1, padding=1),    # N x 1 x 32 x 32
                                nn.ReLU(),
                                nn.Conv2d(1, 1, 3, stride=2, padding=1),    # N x 1 x 16 x 16
                                nn.ReLU())
            
        self.E1 = nn.Sequential(nn.Conv2d(1, 1, 3, stride=2, padding=1),    # N x 1 x 8 x 8
                                nn.ReLU(),
                                nn.ConvTranspose2d(1, 1, 4, stride=2, padding=1),  # N x 1 x 16 x 16
                                nn.ReLU())
                                
        self.N2 = nn.Sequential(nn.Conv2d(1, 1, 3, stride=2, padding=1),    # N x 1 x 8 x 8
                                nn.ReLU())
    ...
    def forward(self, x, use_E_layers=False, get_embedding=False):        
        ...
        out = out.view(out.shape[0], 1, 32, 32)
        out = self.N1(out)
        
        if use_E_layers == False and get_embedding == True:
            out = out.view(out.shape[0], -1)  # Flatten to 2D
...
###########################################################################################################################                
    ...
    def fit_target(self, source_train_set, target_train_set, method=1, use_E_layers=True, variational=True):
        ...
        if method=='finetune-only':
            use_E_layers == False
            self.freeze_layers([0,1])
        else:
            self.freeze_layers([0,2,3])
...
def main():
    ...
    model = Trainer("cnn.pkl", learning_rate_target=0.001, weight_decay_target=0, num_epochs=2)
    ...
    model.fit_target(source_train_set, target_train_set, method='finetune-only')
